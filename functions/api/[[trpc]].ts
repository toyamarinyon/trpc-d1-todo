...
const appRouter = t.router({
  tasks: t.router({
    ...
    create: t.procedure.input(z.object({
      ...
      const result = await ctx.db.prepare('INSERT INTO tasks (title, description) VALUES (?, ?)').bind(input.title, input.description).run()
      if (!result.success) {
    ...
    complete: t.procedure.input(z.object({ id: z.number() })).mutation(async ({ input, ctx }) => {
      const result = await ctx.db.prepare('UPDATE tasks SET completion_datetime = ? WHERE id = ?').bind(new Date().valueOf(), input.id).run()
      if (!result.success) {
        throw new Error(result.error)
      }
    }),
    ...
    list: t.procedure.query(async ({ ctx }) => {
      const result = await ctx.db.prepare('SELECT * FROM tasks WHERE completion_datetime IS NULL').all()
